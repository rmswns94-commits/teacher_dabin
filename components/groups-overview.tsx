...
import { CalendarDays, ChevronRight, Clock3, Search } from "lucide-react";
...
function GroupCard({ group }: { group: GroupCardData }) {
  return (
    ...
    >
      <Card
        className={`h-full p-4 transition hover:-translate-y-0.5 hover:shadow-[0_10px_26px_rgba(0,0,0,0.07)] ${
          group.prepCount > 0 ? "border-[#e8dcc9]" : ""
        } ${group.hasToday ? "border-[#d9d3ef] bg-[#fcfbff]" : ""}`}
      >
        {/* 1. 그룹명 */}
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            {group.scheduleLines.length > 0 ? (
              <div className="mt-1 space-y-0.5 text-xs tabular-nums text-[#6b6b74]">
        ...
        <div className="mt-3 border-t border-[#ececf0] pt-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <Clock3 className="h-3.5 w-3.5 text-[#8b7ae6]" aria-hidden />
            {group.nextLabel ? (
              <>
                <span className="font-semibold tabular-nums text-[#232327]">{group.nextLabel}</span>
                {group.isNow ? (
                ...
                {group.nextSub ? (
                  <span className="text-xs tabular-nums text-[#8a8a93]">{group.nextSub}</span>
                ) : null}
              </>
            ) : (
              <span className="text-sm text-[#8a8a93]">다음 수업 일정 없음</span>
            )}
          </div>
          ...
          <div className="mt-2.5">
            ...
            {group.progressMain ? (
              <>
                <div className="mt-0.5 line-clamp-2 text-sm leading-5 text-[#33333b]">
                  {group.progressMain}
                </div>
                {group.progressSub ? (
                  <div className="line-clamp-1 text-xs text-[#6b6b74]">{group.progressSub}</div>
                ) : null}
              </>
            ) : (
              <div className="mt-0.5 text-sm text-[#9a9aa3]">아직 기록된 진도가 없어요.</div>
            )}
          </div>

          {/* 4. 시험 / 준비 상태 */}
          {group.examLabel || group.prepCount > 0 || group.latestStatus === "draft" ? (
          ...
          {group.latestDateLabel ? (
            <div className="mt-2.5 flex flex-wrap items-center gap-1.5 text-xs text-[#6b6b74]">
            ...
            <div className="mt-2.5 text-xs text-[#9a9aa3]">최근 수업 기록 없음</div>
          )}
        </div>
      </Card>
    </Link>
