...
export function ManualTextbookProgress({ items, textbooks, canAdd, onChange }: {
  ...
  return (
    <div className="min-w-0 space-y-2.5">
      <div className="form-label font-semibold text-[#6652b9]">일반 수업 진도</div>
      {canAdd ? (
        <button type="button" disabled={!hasAvailable}
          ...
          <Plus className="h-4 w-4" aria-hidden /> 교재 진도 추가
        </button>
      ) : null}
      ...
      {items.map((item) => (
        <div key={item.id} data-progress-id={item.id} className="min-w-0 space-y-1.5">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            <button type="button" aria-label={`${item.name || "선택 전 교재"} 진도 삭제`}
          ...
          <textarea value={item.text} rows={3} aria-label={`${item.name || "선택 전 교재"} 진도`}
